...
pub fn demo_seed() -> Result<(), String> {
  ...
  let mk = |id: &str, lead: &serde_json::Value, status: &str, msgs: Vec<(&str, &str, &str, u32, i64)>, followups: u32, next: Option<i64>| {
    ...
    outreach::Thread {
      ...
      followup_count: followups, max_followups: 3, interval_days: 3, next_followup_at: next.map(|m| integrations::iso_from_secs((base + m * 60).max(0) as u64)), last_activity: now.clone(), sequence_id: Some("default".into()), lead: Some(lead.clone()), notes: None, unread: status == "replied", pending_draft: None, pending_reply: None,
    }
  };
  let l = &run1["leads"];
