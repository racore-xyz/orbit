...
When motion affects Arabic, RTL, bilingual Arabic/Latin, or culturally derived ornament, read [references/arabic-mode.md](references/arabic-mode.md) completely before the Motion Audit. Preserve joined Arabic text, semantic DOM order, locale-correct direction, and cultural meaning. Add the reference's bidi, Arabic-type, motif-motion, reduced-motion, and cultural verification checks to the Motion Diff. Never split Arabic into disconnected per-letter DOM spans or reverse arrays/strings to fake RTL movement.
