...
When the page is Arabic, RTL, bilingual Arabic/Latin, or culturally grounded in an Arab region, read [references/arabic-mode.md](references/arabic-mode.md) completely before Phase 1. Add its Arabic mode contract to the Page Read and apply its rules to navigation, section sequence, stats, timelines, forms, tables, cards, locale switching, and the footer. Select one primary heritage lane for the whole page; sections may vary in density, not jump between unrelated civilizations.
