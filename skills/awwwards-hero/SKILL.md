...
When the brief requests Arabic, RTL, bilingual Arabic/Latin, or an Arab-region cultural direction, read [references/arabic-mode.md](references/arabic-mode.md) completely before Phase 1. Add its six-field Arabic mode contract to the Hero Extraction, select exactly one Arabic hero architecture, and run its language, bidi, typography, cultural, and responsive gates in addition to this skill's normal gates. The reference includes researched Islamic, Pharaonic/Nile, desert/Al Sadu, and contemporary Arabic directions. Do not activate it for an unrelated Latin-script hero merely because the client is located in the Middle East.
