...
When the existing product is Arabic, needs an RTL/bilingual mode, or requests an Arab-region visual direction, read [references/arabic-mode.md](references/arabic-mode.md) completely before the Audit. Treat locale routing, translations, direction state, DOM/source order, ids, form values, and data ordering as Sacred. The redesign may replace physical left/right CSS with logical properties and add safe bidi wrappers, but it must not reverse strings, arrays, timelines, or business logic. Add the reference's Arabic engineering and cultural gates to Post-Op verification.
