...
When the generated reference is Arabic, RTL, bilingual Arabic/Latin, or based on Arab-region heritage, read [references/arabic-mode.md](references/arabic-mode.md) completely before Phase 1. Include its six-field contract in the Direction Brief and its exact prompt scaffold in every relevant section prompt. Prefer a clean live-text zone when image generation cannot guarantee correct Arabic shaping. Never accept pseudo-Arabic, invented sacred text, disconnected glyphs, or a collage of unrelated Islamic, Pharaonic, and desert symbols.
